...
const userValidator = { 
    validate: function(fD) { // Validar el formulario en formato FormData
        let errors = [];
        //let pwdPattern = /^([A-Z][0-9])\w{6,}$/; // start with an uppercase letter, then a digit, then at least 6 word characters
        
        let pwdPattern = /^(?=.*[A-Z])(?=(?:.*\d){2,}).{9,}$/; // At least 1 uppercase, 2 digits, min 9 chars
        
        // Current active password pattern:
        // Requires:
        // - (?=.*[A-Z]): At least one uppercase letter.
        // - (?=(?:.*\d){2,}): At least two digits (can be anywhere).
        // - .{9,}: The password must be at least 9 characters long.
        // - Any character is allowed after these conditions are met for the length.
